...
--------------------------------------------
	...
	case $num in
		...
		3)
			echo
			awk -v ADD=${IP} '/("port"|"id"|"path")/{print $1,$2}END{print "IP："ADD}' ${V_CONF}
			manager
		;;
		4)
			echo
			if [ ! -f /etc/mail.rc ];then
				echo -e "${RED}您未配置邮箱,暂时无法使用此功能！${COLOREND}"
			else
				read -p "请输入你的邮箱：" email
				awk -v ADD=${IP} '/("port"|"id"|"path")/{print $1,$2}END{print "IP："ADD}' ${V_CONF} | \ 
				mail -s "v2ray config" ${email}
			fi
			manager
		;;
		5)
			...
			if [[ ${ws} == [Yy] ]];then
				conmand -v nginx &> /dev/null && echo -e "${RED}检测到您已安装Nginx,需要手动配置反向代理！${COLOREND}" || nginx_install
			fi
